...
class ProductsMKLView(ttk.Frame):
    """Справочник товаров для МКЛ с группами и порядком (используются product_groups + products)."""
    ...
    def _reload(self, preserve_state: bool = False, open_gids: set | None = None, select_pref: dict | None = None):
        if preserve_state and (open_gids is None or select_pref is None):
            ...
            if select_pref is None:
                select_pref = sel
        try:
            self._groups = self.db.list_product_groups() if self.db else []
        ...
        try:
            ...
            for g in self._groups:
                self._group_products[g["id"]] = self.db.list_products_by_group(g["id"]) if self.db else []
            self._ungrouped = self.db.list_products_by_group(None) if self.db else []
        except Exception as e:
