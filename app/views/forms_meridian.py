...
class MeridianOrderEditorView(ttk.Frame):
    ...
    def _refresh_items_view(self):
        try:
            ...
            # Do not propagate UI errors; keep form responsive
            pass
    ...
    def _edit_item(self):
        ...
        sph_row = ttk.Frame(frame, style="Card.TFrame"); sph_row.grid(row=3, column=0, sticky="ew", padx=(0,8))
        ttk.Button(sph_row, text="−", width=3, command=lambda: sph_var.set(snap(float(snap(sph_var.get(), -30.0, 30.0, 0.25, allow_empty=True)) - 0.25, -30.0, 30.0, 0.25))).pack(side="left")
        sph_entry = ttk.Entry(sph_row, textvariable=sph_var, width=8, justify="center"); sph_entry.pack(side="left", padx=4)
        ttk.Button(sph_row, text="+", width=3, command=lambda: sph_var.set(snap(float(snap(sph_var.get(), -30.0, 30.0, 0.25, allow_empty=True)) + 0.25, -30.0, 30.0, 0.25))).pack(side="left")
        ...
        cyl_row = ttk.Frame(frame, style="Card.TFrame"); cyl_row.grid(row=3, column=1, sticky="ew", padx=(0,8))
        ttk.Button(cyl_row, text="−", width=3, command=lambda: cyl_var.set(snap(float(snap(cyl_var.get(), -10.0, 10.0, 0.25, allow_empty=True)) - 0.25, -10.0, 10.0, 0.25))).pack(side="left")
        cyl_entry = ttk.Entry(cyl_row, textvariable=cyl_var, width=8, justify="center"); cyl_entry.pack(side="left", padx=4)
        ttk.Button(cyl_row, text="+", width=3, command=lambda: cyl_var.set(snap(float(snap(cyl_var.get(), -10.0, 10.0, 0.25, allow_empty=True)) + 0.25, -10.0, 10.0, 0.25))).pack(side="left")
        ...
        add_row = ttk.Frame(frame, style="Card.TFrame"); add_row.grid(row=5, column=0, sticky="ew", padx=(0,8))
        ttk.Button(add_row, text="−", width=3, command=lambda: add_var.set(snap(float(snap(add_var.get(), 0.0, 10.0, 0.25, allow_empty=True)) - 0.25, 0.0, 10.0, 0.25))).pack(side="left")
        add_entry = ttk.Entry(add_row, textvariable=add_var, width=8, justify="center"); add_entry.pack(side="left", padx=4)
        ttk.Button(add_row, text="+", width=3, command=lambda: add_var.set(snap(float(snap(add_var.get(), 0.0, 10.0, 0.25, allow_empty=True)) + 0.25, 0.0, 10.0, 0.25))).pack(side="left")
        ...
        d_row = ttk.Frame(frame, style="Card.TFrame"); d_row.grid(row=5, column=1, sticky="ew", padx=(0,8))
        def _d_dec():
            cur = snap_int(d_var.get(), 40, 90, step=5, allow_empty=True) or "40"
            try:
                n = int(cur)
            except Exception:
                n = 40
            d_var.set(snap_int(str(n - 5), 40, 90, step=5, allow_empty=True))
        def _d_inc():
            cur = snap_int(d_var.get(), 40, 90, step=5, allow_empty=True) or "40"
            try:
                n = int(cur)
                ...
                n = 40
            d_var.set(snap_int(str(n + 5), 40, 90, step=5, allow_empty=True))
        ttk.Button(d_row, text="−", width=3, command=_d_dec).pack(side="left")
        d_entry = ttk.Entry(d_row, textvariable=d_var, width=8, justify="center"); d_entry.pack(side="left", padx=4)
        ttk.Button(d_row, text="+", width=3, command=_d_inc).pack(side="left")
