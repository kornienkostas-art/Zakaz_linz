...
class MeridianItemForm(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        ttk.Button(d_box, text="−", width=2, style="Tiny.TButton", command=lambda: self._nudge_d(-5)).grid(row=0, column=0, sticky="w")
        self.d_entry = ttk.Entry(d_box, textvariable=self.d_var)
        self.d_entry.grid(row=0, column=1, sticky="ew")
        ttk.Button(d_box, text="+", width=2, style="Tiny.TButton", command=lambda: self._nudge_d(+5)).grid(row=0, column=2, sticky="e")
