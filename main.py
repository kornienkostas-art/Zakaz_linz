...
def _start_tray(master: tk.Tk):
    ...
    menu = pystray.Menu(
        ...
        pystray.MenuItem(
            "Автозапуск: " + ("Выкл" if not _windows_autostart_get() else "Вкл"),
            on_toggle_autostart
        ),
...
class MainWindow(ttk.Frame):
    ...
    def _configure_root(self):
        ...
        if not hasattr(self.master, "app_settings"):
            # Defaults: autostart enabled (Windows), minimize to tray enabled, no logo path yet
        ...
        # Apply autostart setting (Windows only)
        try:
            if os.name == "nt":
...
class SettingsView(ttk.Frame):
    ...
    def _save(self):
        ...
        try:
            ...
            # If tray icon is running and logo changed, restart tray
            try:
...
class MeridianOrderEditorView(ttk.Frame):
    ...
    # ---- Items management for embedded Meridian order editor ----
    def _refresh_items_view(self):
        """Refresh items table from self.items."""
        try:
            for i in self.items_tree.get_children():
                self.items_tree.delete(i)
            for idx, it in enumerate(self.items):
    ...
    def _selected_item_index(self):
        """Return selected row index in items table or None."""
        sel = self.items_tree.selection()
        if not sel:
            messagebox.showinfo("Выбор", "Пожалуйста, выберите позицию.")
            return None
    ...
    def _add_item(self):
        """Open modal form to add item; append to list and refresh."""
        MeridianItemForm(self, on_save=lambda it: (self.items.append(it), self._refresh_items_view()))

    def _edit_item(self):
        """Open modal form to edit selected item."""
        idx = self._selected_item_index()
        if idx is None:
            return
        current = self.items[idx].copy()
    ...
    def _save(self):
        """Collect order data (status + items) and pass to parent on_save, then go back."""
        status = (self.status_var.get() or "Не заказан").strip()
        order = {
            # Title will be generated by parent if empty
            "title": "",
    ...
    def _new_order(self):
        # Открыть встроенную форму нового заказа МКЛ как отдельный вид
        def swap():
            try:
                self.destroy()
            except Exception:
