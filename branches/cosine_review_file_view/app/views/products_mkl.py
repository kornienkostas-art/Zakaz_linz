...
class ProductsMKLView(ttk.Frame):
    """Справочник товаров для МКЛ (отдельная таблица products_mkl)."""
    ...
    def _build_ui(self):
        ...
        header = ttk.Label(table_card, text="Товары (МКЛ)", style="Title.TLabel")
    ...
    def _reload(self):
        try:
            self._dataset = self.db.list_products_mkl() if self.db else []
    ...
    def _on_add_save(self, data: dict):
        try:
            if self.db:
                self.db.add_product_mkl(data.get("name", ""))
            else:
    ...
    def _on_edit_save(self, original_item: dict, data: dict):
        try:
            if self.db and original_item.get("id") is not None:
                self.db.update_product_mkl(original_item["id"], data.get("name", ""))
            else:
        ...
        if messagebox.askyesno("Удалить", "Удалить выбранный товар?"):
            try:
                if self.db and item.get("id") is not None:
                    self.db.delete_product_mkl(item["id"])
                else:
...
class ProductForm(tk.Toplevel):
    def __init__(self, master, initial: dict | None = None, on_save=None):
        ...
        self.title("Карточка товара (МКЛ)")
