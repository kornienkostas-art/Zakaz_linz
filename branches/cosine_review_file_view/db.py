...
class AppDB:
    ...
    def _init_schema(self):
        ...
        # Add comment column if it doesn't exist (SQLite allows ADD COLUMN without IF NOT EXISTS)
        try:
    ...
    def list_clients(self) -> list[dict]:
        rows = self.conn.execute(
            "SELECT id, fio, phone FROM clients ORDER BY fio COLLATE NOCASE;"
        ).fetchall()
        return [{"id": r["id"], "fio": r["fio"], "phone": r["phone"]} for r in rows]

    def add_client(self, fio: str, phone: str) -> int:
        cur = self.conn.execute(
            "INSERT INTO clients (fio, phone) VALUES (?, ?);", (fio, phone)
        )
        self.conn.commit()
    ...
    def update_client(self, client_id: int, fio: str, phone: str):
        self.conn.execute(
            "UPDATE clients SET fio=?, phone=? WHERE id=?;", (fio, phone, client_id)
        )
        self.conn.commit()
    ...
    def update_mkl_order(self, order_id: int, fields: dict):
        ...
        if cols:
            ...
            self.conn.execute(
                f"UPDATE mkl_orders SET {', '.join(cols)} WHERE id=?;", tuple(vals)
            )
            self.conn.commit()
    ...
    def update_meridian_order(self, order_id: int, fields: dict):
        ...
        if cols:
            vals.append(order_id)
            self.conn.execute(
                f"UPDATE meridian_orders SET {', '.join(cols)} WHERE id=?;", tuple(vals)
            )
            self.conn.commit()
