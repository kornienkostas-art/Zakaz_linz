#!/usr/bin/env python3
# Скрипт-установщик проекта "УссурОЧки.рф".
# Скачайте этот файл один (install_ussurochki.py), запустите: python install_ussurochki.py
# Он создаст структуру проекта, запишет все файлы и при желании установит зависимости.
...
PROJECT_FILES = {
    "README.md": """УссурОЧки.рф — настольное приложение для управления заказами
...
def main():
    print("Установщик проекта 'УссурОЧки.рф'")
    default_dir = Path.cwd() / "ussurochki_app"
    target_str = input(f"Куда развернуть проект? [{default_dir}]: ").strip() or str(default_dir)
    target = Path(target_str).expanduser().resolve()

    if target.exists() and any(target.iterdir()):
        if not prompt_yes_no(f\"Папка {target} не пустая. Продолжить и перезаписать файлы?\" ):
            print(\"Отменено.\")
            return

    target.mkdir(parents=True, exist_ok=True)
    write_project(target)
    print(f\"Файлы записаны в: {target}\")
    print(\"Не забудьте положить логотип в assets/logo.png (по желанию).\")

    if prompt_yes_no(\"Установить зависимости (pip install -r requirements.txt)?\"):
        try:
            subprocess.check_call([sys.executable, \"-m\", \"pip\", \"install\", \"-r\", str(target / \"requirements.txt\")])
        except subprocess.CalledProcessError as e:
            print(\"Ошибка установки зависимостей:\", e)
    else:
        print(\"Пропущена установка зависимостей.\")

    if prompt_yes_no(\"Запустить приложение сейчас (python main.py)?\"):
        try:
            subprocess.check_call([sys.executable, str(target / \"main.py\")], cwd=str(target))
        except subprocess.CalledProcessError as e:
            print(\"Ошибка запуска приложения:\", e)
    else:
        print(\"Готово. Для запуска:\")
        print(f\"  cd {target}\")
